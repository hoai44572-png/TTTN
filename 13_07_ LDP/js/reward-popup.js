...
(function () {
    ...
    window.openRewardPopup = function (milestone, code) {
        loadPopupFromHtml().then(function (overlay) {
            if (!overlay) {
                overlay = document.getElementById("rewardPopupOverlay") || document.getElementById("rewardModal");
            }
            if (!overlay) {
                console.warn("[reward-popup] Không tìm thấy phần tử Popup để hiển thị");
                return;
            }

            var milestoneEl = overlay.querySelector("#rewardPopupMilestone") || overlay.querySelector("#rewardModalMilestone");
            ...
            overlay.classList.remove("is-closing", "hide-active");
            overlay.classList.add("is-open", "show");
        });
    };
